...
public class EducationDataService {
	...
	public EducationData save(final EducationData educationData) {
		...
		Assert.isTrue(user.getAuthorities().iterator().next().getAuthority().equals("HACKER"));
		Assert.isTrue(educationData != null && educationData.getDegree() != null && educationData.getInstitution() != null && educationData.getStartDate().before(new Date()) && educationData.getEndDate().before(new Date())
			&& educationData.getEndDate().after(educationData.getStartDate()));
